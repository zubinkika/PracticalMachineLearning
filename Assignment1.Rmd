...
confusionMatrix(newtestData$classe, predict(rf_model,newtestData))
```
The output of the confusion matrix show that the model is **52.28%** accurate.
...
confusionMatrix(newtestData$classe, predict(rf_model,newtestData))
```
The output of the confusion matrix show that the model is **98.11%** accurate.
This looks to be very good model and can be used for final prediction.
...
It can be be concluded that we can use Random forest for the final prediction as it has the highest accuracy.
